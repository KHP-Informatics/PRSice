...
cat(" ################################# \n # \n # \n # \n # \n # PRSice: Polygenic Risk Score software \n # \n # Jack Euesden, Cathryn M. Lewis, Paul F. O'Reilly 2014 \n # If you use PRSice in published work, please cite: Euesden, Lewis, O'Reilly, Bioinformatics (2014) \n # \n # \n # \n #  \n ################################# \n")
...
## preferable
plink <-  NA
ncpu <- NA ## sjn addition . Added --threads option to plink calls. This is only for plink1.9
...
for(basePhen in 1:length(base.phenotypes.names)){
	...
	if(dosage){
	  ...
	  stopifnot(covary = T & !is.na(user.covariate.file))
	...
	if(clump.snps & !dosage & geno.as.list){
			## clump and extract independant snps
			for(chrnum in 1:22){
			  system(paste(plink," --noweb ", " --threads ",ncpu, " --bfile non_synonymous_snps_only_",chrnum," --clump cleaned_base.assoc --clump-p1 ",clump.p1," --clump-p2 ",clump.p2," --clump-r2 ",clump.r2," --clump-kb ",clump.kb," --out cleaned_base_",chrnum, sep=""))
			}
	...
	if(prune.snps){
	  ...
	  if(!dosage & (geno.as.list)){
	    ## prune and extract independant snps
	    for(chrnum in 1:22){
	      system(paste(plink," --noweb ", " --threads ",ncpu, " --bfile non_synonymous_snps_only_",chrnum," --indep-pairwise ",prune.kb.wind," kb ",prune,kb.step," ", prune.kb.r2," --out cleaned_base_",chrnum, sep=""))
	    }
	...
	if(covary & is.na(user.covariate.file)){
	  if(!dosage){
	      ...
	      system(paste(plink, " --noweb ", " --threads ",ncpu, " --bfile ",target, "         --exclude range  mhc.txt   --make-bed  --out target_no_mhc "))
	      system(paste(plink, " --noweb ", " --threads ",ncpu, " --bfile   target_no_mhc     --indep-pairwise 100 25 0.2     --out prune_target"))
	      system(paste(plink, " --noweb ", " --threads ",ncpu, " --bfile ",target, "         --extract prune_target.prune.in    --make-bed   --out prune_target_out"))
	      system(paste(plink, " --noweb ", " --threads ",ncpu, " --bfile   prune_target_out  --genome     --out prune_target_out"))
	      if(ancestry.dim == "MDS"){
	        system(paste(plink, " --noweb ", " --threads ",ncpu, " --bfile   prune_target_out  --read-genome prune_target_out.genome   --cluster --mds-plot", length(covariates), " --out 	  ANCESTRY_INFORMATIVE_DIMENSIONS"))
	        pc.file <- "ANCESTRY_INFORMATIVE_DIMENSIONS.mds"
	      }
	      if(ancestry.dim == "PCA"){
	        system(paste(plink, " --noweb ", " --threads ",ncpu, " --bfile   prune_target_out  --read-genome prune_target_out.genome  --cluster --pca ", length(covariates), " header --out 		ANCESTRY_INFORMATIVE_DIMENSIONS"))
...
system(paste(plink," --noweb ", " --threads ",ncpu, " --bfile ", clump.ref, "  --clump cleaned_base --clump-p1 ",clump.p1," --clump-p2 ",clump.p2," --clump-r2 ",clump.r2," --clump-kb ",clump.kb," --out clumped_base", sep=" "))
